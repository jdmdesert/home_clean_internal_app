-- One-time upgrade for the original Supabase project, which already has a
-- four-column profiles table using the roles "admin" and "cleaner".
begin;

do $$ begin create type public.user_role as enum ('owner', 'employee');
exception when duplicate_object then null; end $$;
do $$ begin create type public.block_status as enum ('open', 'claimed', 'completed', 'cancelled');
exception when duplicate_object then null; end $$;
do $$ begin create type public.occupancy_status as enum ('vacant', 'occupied');
exception when duplicate_object then null; end $$;
do $$ begin create type public.payment_method as enum ('zelle', 'ach', 'check', 'other');
exception when duplicate_object then null; end $$;
do $$ begin create type public.employee_standing as enum ('new', 'good', 'watch', 'risk');
exception when duplicate_object then null; end $$;
...
alter table public.profiles
  ...
  add column if not exists standing public.employee_standing not null default 'new',
...
set role = case when lower(role) in ('admin', 'owner') then 'owner' else 'employee' end,
    ...
    first_name = coalesce(nullif(trim(first_name), ''), split_part(coalesce(full_name, 'Team'), '@', 1)),
    last_name = coalesce(nullif(trim(last_name), ''), 'Account'),
